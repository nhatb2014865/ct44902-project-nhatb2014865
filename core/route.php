...
class route
{
    function __construct()
    {
        ...
        self::handlRoute($folder_root, $__controller, $__action, $__param);
    }

    protected static function handlRoute($folder_root, $__controller, $__action, $__param)
    {
        $url = self::getUrl();

        // $url = self::handlUrl($url);
        ...
        if (!empty($url)) {
            ...
            foreach ($arrUrl as $key => $value) {
                ...
                if (is_file($folder_root . '/' . $value . '.php')) {
                    $__controller = $value;
                    unset($arrUrl[$key++]);
                    if (!empty($arrUrl[$key]) && method_exists($__controller, $arrUrl[$key])) {
                        $__action = $arrUrl[$key];
                        unset($arrUrl[$key]);
                    }
                    break;
    ...
    protected static function handlUrl($url)
    {
        global $__config;
        ...
        if (!empty($__config['route'])) {
            foreach ($__config['route'] as $key => $value) {
                if (preg_match('~' . $key . '~is', $handUrl)) {
                    $handUrl = preg_replace('~' . $key . '~is', $value, $handUrl);
                }
            }
        }
